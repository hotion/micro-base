...
func main() {
	...
	var s service.Service
	{
		...
		// 注册日志中间件
		s = logging.LoggingMiddleware(sugar)(s)
	}
	...
	{
		grpcServer = transport.NewGRPCServer(endpoints, s, sugar)
	}
	...
	go func() {
		...
		if err != nil {
			sugar.Panicf("TCP监听错误:%v", err)
			return
		}
		sugar.Infof("TCP监听成功:%s", config.GetGRPCAddr())
